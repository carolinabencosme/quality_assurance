...
public class UserService {
    ...
    private <T> T withRealm(RealmCallback<T> callback) {
        ...
        try (Keycloak keycloak = clientFactory.create()) {
            ...
            throw ApiException.notFound("Keycloak user not found");
        } catch (BadRequestException ex) {
            throw ApiException.badRequest("Keycloak rejected the user request");
        } catch (Exception ex) {
            log.warn("Keycloak admin API call failed: {}: {}", ex.getClass().getSimpleName(), ex.getMessage());
    ...
    private void validateManagedRoles(Set<String> roles) {
        ...
        for (String role : roles) {
            if (role == null || role.isBlank() || !MANAGED_REALM_ROLES.contains(role)) {
